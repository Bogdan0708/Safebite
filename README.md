...
npm run emu:e2e:stress   # 26 browser scenarios × 3 repeats, retries disabled (flakiness gate)
...
`npm run test:unit` currently reports 241 tests.
...
- Discovery is server-side only: the Places key is a Cloud Functions secret, every callable checks membership first, the field mask asks for id, name, address, Maps link, business status and types only (Pro tier; no coordinates, phone, website or ratings), destination searches are biased to restaurants and every result must be typed as a restaurant, café, bakery, bar or takeaway (`includedType: "restaurant"` with no strict filtering, and the mapper independently keeps only results whose `types` include one of those), and nothing from a Places response is cached or stored — a record created from a result keeps only the place id; the member types the name and address. A deployed function whose secret is the fixture value refuses every search.
