...
npm run emu:e2e:stress   # 25 browser scenarios × 3 repeats, retries disabled (flakiness gate)
...
- Discovery is server-side only: the Places key is a Cloud Functions secret, every callable checks membership first, the field mask asks for id, name, address, Maps link, business status and types only (Pro tier; no coordinates, phone, website or ratings), text searches are restricted to restaurants (`includedType: "restaurant"`, `strictTypeFiltering: true`, and the mapper independently keeps only results whose `types` include `restaurant`), and nothing from a Places response is cached or stored — a record created from a result keeps only the name and address the member saw plus the place id. A deployed function whose secret is the fixture value refuses every search.
