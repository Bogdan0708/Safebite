...
## Addendum — response to the independent review (2026-09-22, `2026-09-22-plan-3-independent-review.md`)

Every finding was verified by the controller before acting (code reading; installed `firebase-functions` logger source; Google's terms pages fetched and read; Places docs and pricing page).
- F1 (late geolocation callback) — confirmed; Fix A b5249c4: page-level search-intent generation; 3 unit regressions + browser scenarios 12–13.
- F2 (prefill in history.state) + S1 (no "user-saved" exception exists) — confirmed: standard terms §3.2.3(a)(iii) forbid copying and saving business names/addresses; Places SST allow only 30-day lat/lng caching; EEA terms defer to nine Permitted Uses. Owner ruling ("go with recommended" = option 1): "Add to our records" carries the place ID only; the member types name and address. Fix E 858511c + 0bdc8f4: form starts empty with a place-id link notice; scenario 10 rewritten, scenario 14 added (reload without saving: form empty, history.state.usr.prefill deep-equals { googlePlaceId }). Invariant shift vs the auditor's probe 3 (which expected prefill undefined): a bare place ID in history state is permitted indefinitely by the Places policy; spec §3.6 records this.
- F3 (unrestricted text search) — confirmed; Fix B e229a72: includedType restaurant, places.types in the mask, mapper type filter. Controller ruling after the fix-wave review (Fix F 952b3a4): strict restaurant-only filtering would hide dedicated gluten-free bakeries/cafés, so no strictTypeFiltering and an allow-list {restaurant, cafe, bakery, bar, meal_takeaway}; Nearby uses the same list. Cost if wrong: a broader result set; one constant to change. Real-Google request/response semantics remain unverified until O4.
- Logger observation — confirmed (positional message overwrites the payload key); Fix C a1c7808: fixed diagnostics only (outcome, status, errorName), tests against the real logger sink (console captured in vi.hoisted); Fix F adds Google's error status from a fixed allow-list (googleStatus).
- Pricing (5,000/SKU/month) and fixture provenance wording — Fix D 0b24ad9; further doc consistency in 4bcd608.
- Reviews: fixes A–E reviewed (opus) — all addressed, one Important product consequence (→ Fix F ruling), minors folded into Fix F; Fix F re-reviewed (sonnet) — clean.
- Gate at 4bcd608: typecheck; web unit 241; functions+rules 260; browser 26/26 (run by the controller after Fix F); guardrail greps clean; secret file ignored.
- Deferred minors from this round: café-titled mapper test has no café fixture entry; pre-deploy history entries holding the old name/address prefill are not scrubbed (no users yet); scenario 13 uses a fixed 2 s negative wait; the two Plan 2b P2 findings and its P3 remain open (owner parked).
